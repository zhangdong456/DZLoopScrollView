...
namespace ScrollView
{
    ...
    public class LoopScrollView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        public float m_bottonPadding = 0f;
        public float m_topPadding = 0;
        public float m_Spacing = 0f; //间距
        public GameObject m_CellGameObject; //指定的cell

        public Action<GameObject,int> m_SpeicalCellAction;//特殊效果处理
        ...
        private bool isUpdateCheckVectory = false;
        //记录 物体的坐标 和 物体 
        ...
        protected void UpdateRefreshSpecitalCell()
        {
            for (int i = 0, length = m_CellInfos.Length; i < length; i++)
            {
                CellInfo cellInfo = m_CellInfos[i];
                if (cellInfo.obj != null)
                {
                    ...
                    if (!IsOutRange(rangePos))
                    {
                        Func(m_SpeicalCellAction, cellInfo.obj, true);
                    }
                }
            }
        }
        ...
        private float GetPaddingValue()
        {
            return m_topPadding + m_bottonPadding;
        }
        ...
        public void DeleteItem(int index)
        {
            ...
            SetPoolsObj(m_CellInfos[index].obj);
            CellInfo[] newCellInfos = new CellInfo[m_CellInfos.Length - 1];
            for (int i = 0, length = m_CellInfos.Length; i < length; i++)
            {
                if (i < index)
                {
                    newCellInfos[i] = m_CellInfos[i];
                }
                else if (i > index)
                {
                    newCellInfos[i - 1] = m_CellInfos[i];
                }
            }
            m_CellInfos = newCellInfos;
            int num = m_CellInfos.Length;
            if (m_Direction == e_Direction.Vertical)
            {
                float contentSize = (m_Spacing + m_CellObjectHeight) * Mathf.CeilToInt((float)(num) / m_Row)+ GetPaddingValue();
            ...
            {
                float contentSize = (m_Spacing + m_CellObjectWidth) * Mathf.CeilToInt((float)num / m_Row)+ GetPaddingValue();
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                if (i>=index )
                {
                    float pos1 = 0;  //坐标( isVertical ? 记录Y : 记录X )
                    float rowPos1 = 0; //计算每排里面的cell 坐标
                    if(m_Direction == e_Direction.Vertical)
                    {
                        pos1 = m_CellObjectHeight * Mathf.FloorToInt(i / m_Row) + m_Spacing * Mathf.FloorToInt(i / m_Row)+m_topPadding;
                        rowPos1 = m_CellObjectWidth * (i % m_Row) + m_Spacing * (i % m_Row);
                        tempCellInfo.pos = new Vector3(rowPos1, -pos1, 0);
                    }
                    else
                    {
                        pos1 = m_CellObjectWidth * Mathf.FloorToInt(i / m_Row) + m_Spacing * Mathf.FloorToInt(i / m_Row)+m_topPadding;
                        rowPos1 = m_CellObjectHeight * (i % m_Row) + m_Spacing * (i % m_Row);
                        tempCellInfo.pos = new Vector3(pos1, -rowPos1, 0);
                    }
                    
                    
                    //-> 计算是否超出范围
                    float rPos = m_Direction == e_Direction.Vertical ? tempCellInfo.pos.y : tempCellInfo.pos.x;
                    if (!IsOutRange(rPos))//在显示范围内
                    {
        ...
        public virtual void ShowList(int num)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                if(m_Direction == e_Direction.Vertical)
                {
                    pos = m_CellObjectHeight * Mathf.FloorToInt(i / m_Row) + m_Spacing * Mathf.FloorToInt(i / m_Row)+m_topPadding;
                    rowPos = m_CellObjectWidth * (i % m_Row) + m_Spacing * (i % m_Row);
                    cellInfo.pos = new Vector3(rowPos, -pos, 0);
                }
                else
                {
                    pos = m_CellObjectWidth * Mathf.FloorToInt(i / m_Row) + m_Spacing * Mathf.FloorToInt(i / m_Row)+m_topPadding;
